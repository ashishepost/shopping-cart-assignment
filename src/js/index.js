...
const productsController = () => {
  ...
  fetchData(APIEndPoints.baseURL + APIEndPoints.categories.URL, null, 'GET')
    .then(data => {
      if (!servicesData.categories) {
        servicesData.categories = data;
      }
    })
    .catch(error => console.error(error));

  fetchData(APIEndPoints.baseURL + APIEndPoints.products.URL, null, 'GET')
    .then(data => {
      if (!servicesData.products) {
        // console.log(data);
        servicesData.products = data;
        renderCarousel(false);
        renderProducts(false);
        XHRLoader(false);
      }
...
const initApp = () => {
  ...
  registerRoutes();
  registerMessages();
  fetchData(APIEndPoints.baseURL + APIEndPoints.banners.URL, null, 'GET')
    .then(data => {
      if (!servicesData.banners) {
        servicesData.banners = data;
        renderCarousel(true);
        XHRLoader(false);
      }
    })
